...
export default function GiftPage() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (giftSearchQuery) {
      filtered = filtered.filter((o) =>
        o.rewardName.toLowerCase().includes(giftSearchQuery.toLowerCase()),
      );
    }
    if (giftDate) {
      const filterDate = giftDate.toDateString();
      filtered = filtered.filter(
        (o) => new Date(o.createdAt).toDateString() === filterDate,
      );
    }
  ...
  return (
    <>
      ...
      >
        {showOrderDetail && (
          <div className='flex flex-col overflow-hidden bg-white'>
            <div className='relative overflow-hidden bg-gradient-to-br from-sky-50 via-white to-amber-50 px-6 pt-5 pb-5'>
      ...
      <div className='mb-4 flex flex-col gap-3 rounded-[28px] border border-white/65 bg-white/72 p-4 text-slate-800 shadow-[0_14px_36px_rgba(15,23,42,0.08)] backdrop-blur-xl sm:flex-row sm:items-center'>
        <div>
      ...
      {/* 礼物筛选区 */}
      <div className='mb-4 space-y-3 rounded-[28px] border border-white/65 bg-white/72 p-4 shadow-[0_14px_36px_rgba(15,23,42,0.08)] backdrop-blur-xl'>
        ...
        <div className='flex overflow-x-auto rounded-[18px] border border-slate-200/70 bg-white/80 p-1 backdrop-blur'>
          {(['all', 'pending', 'verified', 'cancelled'] as const).map(
            (status) => (
              <Button
                key={status}
                onClick={() => setGiftStatusFilter(status)}
                variant='secondary'
                className={`flex-1 whitespace-nowrap rounded-[1rem] border-none py-2 text-xs font-bold transition md:text-sm ${
                  giftStatusFilter === status
                    ? 'bg-slate-900 text-white shadow-[0_8px_18px_rgba(15,23,42,0.16)]'
                    : 'bg-transparent text-slate-500 hover:-translate-y-px hover:bg-slate-50 hover:text-slate-900 hover:shadow-[0_8px_18px_rgba(15,23,42,0.08)]'
                }`}
              >
                {status === 'all'
                  ? '全部'
      ...
      <div className='grid grid-cols-2 gap-3'>
        {filteredOrders.length > 0 ? (
          filteredOrders.map((order) => (
            <div
              key={order._id}
              className={`group relative flex cursor-pointer flex-col items-center gap-2 overflow-hidden rounded-[22px] border p-3 text-center transition active:scale-[0.98] ${
                order.status === 'verified'
                  ? 'border-emerald-100 bg-emerald-50/70 shadow-[0_10px_24px_rgba(16,185,129,0.08)]'
                  : order.status === 'cancelled'
                    ? 'border-slate-200/70 bg-white/75 opacity-80'
                    : 'border-slate-200/70 bg-white shadow-[0_10px_24px_rgba(15,23,42,0.06)]'
              }`}
            ...
            >
              ...
              {order.rewardIcon ? (
                <div className='flex h-14 w-14 items-center justify-center rounded-[18px] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.08)] ring-1 ring-slate-100 text-3xl'>
                  {order.rewardIcon}
                </div>
              ) : null}
              <h3 className='line-clamp-1 text-sm font-black tracking-tight text-slate-900'>
                {order.rewardName}
              </h3>
          ...
          <div className='col-span-2 rounded-[24px] border border-dashed border-slate-200 bg-white/75 py-14 text-center text-slate-500 shadow-[0_10px_28px_rgba(15,23,42,0.05)]'>
            <Gift size={48} className='mx-auto mb-3 text-slate-300' />
            <p className='font-semibold text-slate-600'>还没兑换过礼物哦</p>
            <Button
              onClick={() => navigateTo('store')}
              variant='secondary'
              className='mt-3 border-none bg-transparent text-sky-600 shadow-none'
            >
              去商城看看
            </Button>
          </div>
