...
export default function ChildHome() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/tasks?page=1&limit=10', {
  ...
  const handleSubmitTask = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        fetchTasks();
        showToastMessage("提交成功！等待家长审核~");
      } else {
        showToastMessage("提交失败，请重试");
      }
    } catch (error) {
  ...
  const handleQuickComplete = async (task: Task) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        fetchTasks();
        showToastMessage(`任务提交成功！+${task.points}分`);
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="relative min-h-screen text-gray-800">
        ...
        <AnimatePresence>
          {showToast && (
            <motion.div 
              ...
              className="fixed top-20 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-md px-6 py-3 rounded-full shadow-2xl z-[200] font-bold text-gray-800 border-2 border-yellow-400"
            >
              {showToast}
            </motion.div>
          )}
        ...
        {/* 家长模式弹窗 */}
        {showParentModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-6" onClick={() => setShowParentModal(false)}>
            <div className="glass-strong rounded-3xl p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-black text-gray-800">家长验证</h3>
                <button onClick={() => setShowParentModal(false)} className="text-gray-400 hover:text-gray-600">✕</button>
              </div>
