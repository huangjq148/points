      ...
      <div className="fixed top-4 right-4 flex flex-col gap-2 pointer-events-none" style={{ zIndex: "var(--z-toast)" }}>
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`
              pointer-events-auto flex items-center gap-3 px-4 py-3 transform transition-all duration-300 animate-slide-in
              ${CONTROL_PANEL_RADIUS_CLASS} ${CONTROL_PANEL_CLASS}
            `}
