...
export default function ParentDashboard() {
  ...
  const [newReward, setNewReward] = useState({ name: '', points: 50, type: 'physical', icon: '🎁' });
  ...
  const fetchTasks = async () => {
    ...
    if (data.success) {
      const tasksWithNames = await Promise.all(data.tasks.map(async (task: any) => {
  ...
  const fetchOrders = async () => {
    ...
    const data = await res.json();
    if (data.success) {
      const ordersWithNames = await Promise.all(data.orders.map(async (order: any) => {
  ...
  const handleAddReward = async () => {
    ...
    if (data.success) {
      ...
      setNewReward({ name: '', points: 50, type: 'physical', icon: '🎁' });
      fetchRewards();
    } else {
  ...
  return (
    <div className="dashboard-layout">
      ...
      <div className="main-content">
                ...
                {childList.map((child) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500">积分: {child.availablePoints}</p>
                    </div>
        ...
        {activeTab === 'audit' && (
          <>
            <h2 className="text-lg font-bold text-gray-800 mb-4">任务审核</h2>
